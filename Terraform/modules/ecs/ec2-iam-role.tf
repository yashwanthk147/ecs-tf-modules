...
resource "aws_iam_role" "ecs_role" {
  name_prefix        = "${aws_ecs_cluster.ecs_cluster.name}-ec2-role"
...
resource "aws_iam_policy" "ecs_role_permissions" {
  name_prefix = "${aws_ecs_cluster.ecs_cluster.name}-ecs-policy"
...
resource "aws_iam_policy_attachment" "ecs_instance_role_policy_attachment" {
  name = "${aws_ecs_cluster.ecs_cluster.name}-iam-policy-attachment"
  roles = [
